    ...
    if (token.type == kh::TokenType::SYMBOL &&
        ...
        for (auto generic_ : id_generic_args->generics) {
            if (generic_->identifiers.size() != 1)
                this->exceptions.emplace_back(
                    U"Could not have multiple identifiers as a generic argument name",
                    generic_->index);
            if (!generic_->generics.empty())
                this->exceptions.emplace_back(
                    U"Could not have generic arguments in a generic argument",
    ...
    while (true) {
        ...
        if (token.type == kh::TokenType::SYMBOL) {
            if (token.value.symbol_type == kh::Symbol::COMMA) {
                ...
                this->exceptions.emplace_back(U"Was expecting a closing parentheses or a comma",
                                              token.index);
                goto end;
            }
        }
...
kh::AstDeclarationExpression* kh::Parser::parseDeclaration(const bool is_static,
    ...
    return new kh::AstDeclarationExpression(index, var_type, var_name, expression, ref_depth,
                                            is_static, is_public);
}
...
kh::AstClass* kh::Parser::parseClass() {
    ...
    if (token.type == kh::TokenType::SYMBOL &&
        token.value.symbol_type == kh::Symbol::GENERIC_OPEN) {
        this->ti++;
        GUARD(0);
        token = this->to();
        ...
        while (true) {
            ...
            if (token.type == kh::TokenType::SYMBOL &&
                ...
                this->exceptions.emplace_back(U"Was expecting a comma or a generic close",
                                              token.index);
                break;
            }
            ...
            switch (token.type) {
                case kh::TokenType::IDENTIFIER: {
                    if (token.value.identifier == U"var" || token.value.identifier == U"ref" ||
                        token.value.identifier == U"static" ||
                        token.value.identifier == U"private" ||
                        ...
                        if (token.type == kh::TokenType::SYMBOL &&
                            ...
                            this->ti++;
                            this->exceptions.emplace_back(U"Was expecting a semicolon",
                                                          token.index);
                        }
                    }
        ...
        switch (token.type) {
            case kh::TokenType::IDENTIFIER: {
                if (token.value.identifier == U"def") {
                    ...
                    if (token.type == kh::TokenType::IDENTIFIER &&
                        token.value.identifier == U"while") {
                        this->ti++;
                    ...
                    while (true) {
                        ...
                        if (token.type == kh::TokenType::SYMBOL) {
                            switch (token.value.symbol_type) {
                                ...
                                default:
                                    this->exceptions.emplace_back(U"Was expecting a colon",
                                                                  token.index);
                            }
                        }
                        else
                ...
                switch (token.value.symbol_type) {
                    ...
                    case kh::Symbol::DOLLAR: {
                        ...
                        if (token.type == kh::TokenType::IDENTIFIER)
                            ...
                            this->exceptions.emplace_back(U"Was expecting an identifier",
                                                          token.index);
                        ...
                        while (true) {
                            ...
                            if (token.type == kh::TokenType::SYMBOL &&
                                ...
                                break;
                                       ...
                                       token.value.symbol_type == kh::Symbol::COMMA)) {
                                this->exceptions.emplace_back(
                                    U"Was expecting a semicolon or a comma", token.index);
                                break;
                            }
...
kh::AstExpression* kh::Parser::parseExpression() {
    ...
    if (token.type == kh::TokenType::IDENTIFIER &&
        (token.value.identifier == U"var" || token.value.identifier == U"ref" ||
    ...
    while (token.type == kh::TokenType::OPERATOR) {
        switch (token.value.operator_type) {
            ...
            case kh::Operator::IPOW: {
                ...
                expr =
                    new kh::AstBinaryExpression(token.index, token.value.operator_type, lval, rval);
            } break;
