...
std::vector<std::u32string>* errors_ptr;

static void utf8EncodeTest() {
    KH_TEST_ASSERT(kh::encodeUtf8(KH_TEST_U32STRING) == KH_TEST_U8STRING);
    return;
error:
    errors_ptr->push_back(U"Assertion error in utf8EncodeTest");
}
...
static void utf8DecodeTest() {
    ...
    return;
error:
    errors_ptr->push_back(U"Assertion error in utf8DecodeTest");
}

void kh_test::utf8Test(std::vector<std::u32string>& errors) {
    errors_ptr = &errors;
    utf8EncodeTest();
    utf8DecodeTest();
}
