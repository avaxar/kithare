...
khArray_char kh_quoteString(khArray_char* string) {
    ...
    khArray_char_push(&quoted_string, '\"');
    for (uint32_t* chr = string->array; chr < string->array + string->size; chr++) {
...
khArray_char kh_quoteBuffer(khArray_byte* buffer) {
    ...
    khArray_char_push(&quoted_buffer, 'b');
    khArray_char_push(&quoted_buffer, '\"');
...
khArray_char kh_uintToString(uint64_t uint_v, uint8_t base) {
    ...
    while (value > 0) {
        ...
        khArray_char_push(&string, (digit < 10 ? '0' + digit : 'A' + digit - 10));
        value /= base;
    }
